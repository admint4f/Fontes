...
//ณ mv_par07				// Compoen saldo por				ณ
//ณ mv_par08				// Considera Filial					ณ
//ณ mv_par09				// Da Filial						ณ
//ณ mv_par10				// Ate Filia						ณ
...
If oReport:lXlsTable
	ApMsgAlert(STR0055) //"Formato de impressใo Tabela nใo suportado neste relat๓rio"
	oReport:CancelPrint()
	Return
Endif
...
If Empty(mv_par03)
	HELP (" ",1,'DTDEBITO',,"Data de dbito n?o informada na parametriza?o do relatขrio.",1,0,,,,,,{"Por favor, informe a data de dbito nos parmetros do relatขrio (pergunte)."} ) 	//###
	oReport:CancelPrint()
	Return
Endif
...
If SE2->(MsSeek(xFilial("SE2", (cAliasSea)->EA_FILORIG)+(cAliasSea)->(EA_PREFIXO+EA_NUM+EA_PARCELA+EA_TIPO+EA_FORNECE+EA_LOJA)))
	...
	If SE5->(MsSeek(xFilial("SE5", (cAliasSea)->EA_FILORIG)+"BA"+SE2->(E2_PREFIXO+E2_NUM+E2_PARCELA+E2_TIPO)+DTOS(SE2->E2_BAIXA)+SE2->(E2_FORNECE+E2_LOJA)))
		...
		//Posiciono no primeiro movimento encontrado ou no ฃltimo movimento n?o cancelado,  importante manter a SE5 com a chave do tกtulo da SE2
		SE5->(dbGoTo(nRecNo))
	Endif
EndIf
...
//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
//ณ Vari veis utilizadas para Impresso do Cabealho e Rodap	  ณ
//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
cbtxt 	:= SPACE(10)
cbcont	:= 0
li 		:= 80
...
//ณ Atribui valores as variaveis ref a filiais                ณ
	...
	While !Eof() .And. cFilial == EA_FILIAL .And. EA_NUMBOR <= mv_par02
		...
		If !lBaixa
			...
			If (dbSeek( sFilial+"BA"+SE2->E2_PREFIXO+SE2->E2_NUM+SE2->E2_PARCELA+SE2->E2_TIPO+DtoS(SE2->E2_BAIXA)+SE2->E2_FORNECE+SE2->E2_LOJA))
				...
				//Posiciono no primeiro movimento encontrado ou no ๚ltimo movimento nใo cancelado, ้ importante manter a SE5 com a chave do tํtulo da SE2
		...
		//ณ Verifica se no h  mais registros v lidos a analisar.    ณ
...
// Parโmetro que se passado suprime o texto padrao desta fun็ใo por outro customizado
...
If !Empty((cAliasQuery)->EA_FILORIG)//Se possui EA_FILORIG, utilizo esse campo na chave de busca
	...
	While !SE2->(Eof()) .And. (cAliasQuery)->(EA_FILORIG+EA_PREFIXO+EA_NUM+EA_PARCELA+EA_TIPO+EA_FORNECE+EA_LOJA) ==;
	 ...
	 SE2->(dbSkip())
	EndDo
Else//Valida็ใo anterior, caso o EA_FILORIG esteja em branco
	dbSelectArea("SE2")
	SE2->(dbSetOrder(1))
