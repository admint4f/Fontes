...
Static Function fRunSPT(_oReport,aCodRet)
	...
	TRCell():New(_oSec1,"CGC"    ,"TRB",STR0023,/*Picture*/, 25 ,/*lPixel*/,{ || If(!Empty(TRB->CGC), TRB->CGC, ) } )	//"CGC"
...
Static Function _FGrvTRB(_oSec1,aCodRet)
	...
	cQuery += " AND SE2.E2_PIS = 0 "
	cQuery += " AND SE2.E2_COFINS = 0 "
	cQuery += " AND SE2.E2_CSLL = 0 "
	cQuery += " AND SE2.E2_SEST = 0 "  

	If lContRetIRF
		cQuery += " AND (SE2.E2_IRRF = 0 OR (SE2.E2_VRETIRF = 0 AND SE2.E2_PRETIRF <> '1' ))) "
	Else 
	...
	cQuery += " ORDER BY CODIGO,LOJA"
	cQuery := ChangeQuery(cQuery)

	TcQuery cQuery New Alias "TMPQTRB"

	If TMPQTRB->(!EOF())
		While TMPQTRB->(!EOF())

			If	lContrRet
				nValBase := TMPQTRB->(VALBASE+VALIRRF+VALISS)+IF((TMPQTRB->DEDINSS=="1"),TMPQTRB->VALINSS,0)
				nValLiq := TMPQTRB->VALBASE	
				If (Empty(TMPQTRB->PRETPIS) .OR. Empty(TMPQTRB->PRETCOF) .OR. Empty(TMPQTRB->PRETCSL)) .AND. (TMPQTRB->VALPIS+TMPQTRB->VALCOF+TMPQTRB->VALCSLL) > 0
					//PCC retido na emissao nele mesmo, somar para compor o valor original.
					nValBase += TMPQTRB->VALPIS+TMPQTRB->VALCOF+TMPQTRB->VALCSLL
				Else
					...
					nValLiq -= TMPQTRB->VALPIS+TMPQTRB->VALCOF+TMPQTRB->VALCSLL
				Endif
			Else
				nValBase := TMPQTRB->(VALBASE+VALIRRF+VALISS+VALPIS+VALCOF+VALCSLL)+IF((TMPQTRB->DEDINSS=="1"),TMPQTRB->VALINSS,0)
				nValLiq := TMPQTRB->VALBASE
			EndIf
			nValBase += TMPQTRB->VALSEST

			dbSelectArea("TRB")
			If RecLock("TRB",.T.)	
				TRB->CODIGO		:= TMPQTRB->CODIGO
				TRB->LOJA		:= TMPQTRB->LOJA
				TRB->NOMEFOR	:= TMPQTRB->NOMEFOR
				TRB->CGC		:= TMPQTRB->CGC
				TRB->FILIAL  	:= TMPQTRB->FILIAL
				TRB->PREFIXO	:= TMPQTRB->PREFIXO
				TRB->NUM		:= TMPQTRB->NUM
				TRB->PARCELA	:= TMPQTRB->PARCELA
				TRB->TIPO		:= TMPQTRB->TIPO
				TRB->EMISSAO	:= StoD(TMPQTRB->EMISSAO)
				TRB->VENCTO		:= StoD(TMPQTRB->VENCTO)
				TRB->VALBASE	:= nValBase
				TRB->VALINSS	:= TMPQTRB->VALINSS
				TRB->VALPIS		:= TMPQTRB->VALPIS
				TRB->VALCOF		:= TMPQTRB->VALCOF
				TRB->VALCSLL	:= TMPQTRB->VALCSLL 
				TRB->VALIRRF	:= TMPQTRB->VALIRRF
				TRB->VALISS		:= TMPQTRB->VALISS       
				TRB->FATURA		:= TMPQTRB->FATURA
				IF lCalcIssBx
					TRB->TRETISS	:= TMPQTRB->TRETISS 			
				EndIf
				TRB->PARCISS	:= TMPQTRB->PARCISS
				TRB->PARCINS	:= TMPQTRB->PARCINS
				TRB->PARCIR		:= TMPQTRB->PARCIR
				TRB->PARCPIS	:= TMPQTRB->PARCPIS
				TRB->PARCCOF	:= TMPQTRB->PARCCOF
				TRB->PARCSLL	:= TMPQTRB->PARCSLL
				TRB->PARCSES	:= TMPQTRB->PARCSES
				TRB->NATUREZ	:= TMPQTRB->NATUREZ
				TRB->VALSEST	:= TMPQTRB->VALSEST  
				TRB->VALLIQ		:= nValLiq
				TRB->FILORI		:= TMPQTRB->FILORIG
				TRB->MUNIC		:= TMPQTRB->MUNIC
				TRB->TPNOT		:= TMPQTRB->TPNOT
				//Se controla retencao
				If lContrRet
					TRB->VRETPIS	:= TMPQTRB->VRETPIS
					TRB->VRETCOF	:= TMPQTRB->VRETCOF
					TRB->VRETCSL	:= TMPQTRB->VRETCSL 
					TRB->PRETPIS	:= TMPQTRB->PRETPIS
					TRB->PRETCOF	:= TMPQTRB->PRETCOF
					TRB->PRETCSL	:= TMPQTRB->PRETCSL 
				Endif			
